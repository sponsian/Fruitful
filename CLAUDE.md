...
Fruitful is a fork of Smol (SmolDapp), rebranded and ported to the **Reef Pelagia testnet** (chain ID 13939, native REEF with 12 decimals, PolkaVM via `pallet-revive`) as the **only** supported network. Ethereum mainnet was removed so the app never contacts an Ethereum RPC. As a consequence, **ENS name/avatar resolution is disabled app-wide** (ENS is Ethereum-only) — the `useENS` hook is a no-op, the `useEnsName`/`useEnsAvatar` hooks are disabled, and the `getEnsName`/`getEnsAddress` action calls were removed; Clusters name resolution still works via its own API. The **swap** (LiFi) and **multisafe** (Safe) apps don't work on Pelagia and are hidden from the UI (side-nav + homepage), though their routes still exist. Internal identifiers still use the Smol naming (`Smol*` components, `TSmol*` types, `SMOL_*` env vars) — only user-visible branding was renamed. See `docs/pelagia-port-plan.md` and `docs/pelagia-contract-deployment.md`.
...
`app/_utils/tools.chains.ts` is the single source of truth for supported networks (currently **Reef Pelagia only** — Ethereum mainnet was removed, see Project). `CHAINS` is a record keyed by chain id where each entry extends a viem `Chain` with app metadata (`isEnabledInProd`, `isLifiSwapSupported`, `isMultisafeSupported`, `safeAPIURI`, `disperseAddress`, etc.). It exports `networks` and `supportedNetworks`. Add/modify chain support here, plus the matching `RPC_URI_FOR_<id>` env entry in `next.config.js`. Testnet chains flagged `isEnabledInProd` (Reef Pelagia) are treated as user-facing networks; `useWallet` skips other testnets outside dev. The Pelagia Multicall3 (`contracts.multicall3`) and Disperse (`disperseAddress`) contracts are already deployed and wired in (see `docs/pelagia-contract-deployment.md`); the disperse button auto-disables whenever `disperseAddress` is the zero address, and balance reads need `contracts.multicall3`, so both must stay set. Pelagia state may be reset on upgrades — if the contracts disappear, redeploy and update the addresses here.
